...
class WADocumentDownloader():
    ...
    def _decrypt(self, ciphertext, ref_key, media_info):
        ...
        progress = self._create_progress_iterator(
            range(length_kb), length_kb, "Decrypt        ")

        try:
            plaintext = self._media_cipher.decrypt(
                ciphertext, ref_key, media_info)
    ...
    def _write(self, data, filename):
        ...
        progress = self._create_progress_iterator(
            range(length_kb), length_kb, "Write          ")
        try:
    ...
    def run(self, media_message_protocolentity):
        logger.debug(
        ...
        while True:
            ...
            if isinstance(media_message_protocolentity, DownloadableMediaMessageProtocolEntity):
                logger.info(
                    ...
                    (media_message_protocolentity.url,
                     base64.b64encode(media_message_protocolentity.media_key))
                    )
            else:
                logger.info("Processing %s" % media_message_protocolentity.media_type)
                print("Processing %s" % media_message_protocolentity.media_type)

            filedata = None
            fileext = None

            # image
            if isinstance(media_message_protocolentity,
                          ImageDownloadableMediaMessageProtocolEntity):
                media_info = MediaCipher.INFO_IMAGE
                filename = "image"

            # audio
            elif isinstance(media_message_protocolentity,
                            AudioDownloadableMediaMessageProtocolEntity):
            ...
            if filedata is None:
                enc_data = self._download(media_message_protocolentity.url.strip())

                if enc_data is None:
                    logger.error("Download failed")
                    sys.exit(1)

                filedata = self._decrypt(enc_data,
                                         media_message_protocolentity.media_key,
                                         media_info)
                if filedata is None:
                    logger.error("Decrypt failed")
                    sys.exit(1)

            if not isinstance(media_message_protocolentity,
                              DocumentDownloadableMediaMessageProtocolEntity):
                if fileext is None:
