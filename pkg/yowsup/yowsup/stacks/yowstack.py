...
import time, logging, random, sys
...
class YowStack(object):
    ...
    def emitEvent(self, yowLayerEvent):
        if not self.__stackInstances[0].onEvent(yowLayerEvent):
            logging.info('__stackInstances - {}'.format(self.__stackInstances))
            logging.info('EMIT EVENT - {}'.format(yowLayerEvent))
    ...
    def loop(self, *args, **kwargs):
        while True:
            try:
                ...
                logging.info('YOWSTACK loop callback {}'.format(callback))
                callback()
            except Queue.Empty:
                logging.info('Emtpy Queue')
